...
export class RedisService {
  ...
  async createRoom(room: Room): Promise<void> {
    ...
    await this.client!.sadd('rooms:active', room.id);
  }
  ...
  async findRoomByInviteCode(inviteCode: string): Promise<string | null> {
    ...
    for (const roomId of roomIds) {
      const code = await this.client!.hget(`room:${roomId}`, 'inviteCode');
      if (code === inviteCode) {
        return roomId;
      }
    }
  ...
  async getRoomPlayers(roomId: string): Promise<RoomPlayer[]> {
    const data = await this.client!.hgetall(`room:${roomId}:players`);
  ...
  async updateRoomPlayer(
    ...
    const current = await this.client!.hget(`room:${roomId}:players`, playerId);
    if (!current) return;
  ...
  async validateReconnectToken(
    ...
    const data = await this.client!.get(`reconnect:${token}`);
    if (!data) return null;
    await this.client!.del(`reconnect:${token}`);
  ...
  async updatePresence(playerId: string): Promise<void> {
    await this.client!.zadd('presence:online', Date.now(), playerId);
  }

  async removePresence(playerId: string): Promise<void> {
    await this.client!.zrem('presence:online', playerId);
  }

  async getOnlinePlayers(since: number = Date.now() - 60000): Promise<string[]> {
    return this.client!.zrangebyscore('presence:online', since, '+inf');
  }
  ...
  async removeFromMatchmakingQueue(
    mode: string,
    playerId: string
  ): Promise<void> {
    // Get all entries and filter out the player
    const entries = await this.client!.zrange(`matchmaking:queue:${mode}`, 0, -1);
    for (const entry of entries) {
      const data = JSON.parse(entry);
      if (data.playerId === playerId) {
        await this.client!.zrem(`matchmaking:queue:${mode}`, entry);
        break;
      }
    }
  }
  ...
  async getMatchmakingQueue(mode: string, count: number = 10): Promise<any[]> {
    const entries = await this.client!.zrange(`matchmaking:queue:${mode}`, 0, count - 1);
